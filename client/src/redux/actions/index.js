...
export const getDiets = () => async (dispatch) => {
    try {
        const diets = await axios.get('https://pifood-production-911b.up.railway.app/diets');
...
export const getAllRecipes = () => {
    ...
    try {
        const response = await axios.get('https://pifood-production-911b.up.railway.app/recipes')//coneccion conel back
...
export const getRecipesName = (name) => {
    return async (dispatch) => {
        try {
           const response =  await axios.get(`https://pifood-production-911b.up.railway.app/recipes?name=${name}`)
           dispatch({
...
export const getRecipe = (id) => {
    return async (dispatch) => {
        try {
            const response = await axios.get(`https://pifood-production-911b.up.railway.app/recipes/${id}`)
                dispatch({
                    type: 'GET_RECIPE',
                    payload: response.data
                })
...
export const createRecipes = (payload) => { 
    return async function () {
        let response = await axios.post('https://pifood-production-911b.up.railway.app/recipes', payload) 
    return response 
    } 
 }
